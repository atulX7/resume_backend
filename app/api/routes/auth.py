...
async def sync_user(user_data: SyncUserRequest, db: Session = Depends(get_db)):
    ...
    if not user_data.access_token:
        ...
        raise HTTPException(
            status_code=401, detail="Failed to login. Please try again."
        )
    ...
    try:
        ...
        logger.info(
            f"User {user['user_id']} synced successfully with plan: {user['plan_code']}"
        )
        return user
    except Exception as e:
        logger.error(f"Exception while syncing user: {str(e)}")
        raise HTTPException(
            status_code=500, detail="Internal error while syncing user."
        )
