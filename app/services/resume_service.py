...
def handle_resume_upload(db: Session, user_id: str, file: UploadFile, title: str):
    ...
    try:
        ...
        logger.info(
            f"[UPLOAD_RESUME] Completed for user: {user_id}, resume_id: {resume.id}"
        )
        return resume

    except Exception as e:
        logger.error(
            f"[UPLOAD_RESUME] Failed for user: {user_id} - {str(e)}", exc_info=True
        )
        raise
...
def handle_delete_resume(db: Session, resume_id: str):
    ...
    try:
        ...
        logger.error(
            f"[DELETE_RESUME] Failed for resume_id: {resume_id} - {str(e)}",
            exc_info=True,
        )
        raise
...
def get_resume_download_url(db: Session, user_id: str, resume_id: str):
    ...
    logger.info(
        f"[DOWNLOAD_URL] Requested by user: {user_id} for resume_id: {resume_id}"
    )
    ...
    try:
        ...
        if not resume or resume.user_id != user_id:
            logger.warning(
                f"[DOWNLOAD_URL] Resume not found or access denied for resume_id: {resume_id}"
            )
            return None
        ...
        logger.error(
            f"[DOWNLOAD_URL] Failed to generate URL for resume_id: {resume_id} - {str(e)}",
            exc_info=True,
        )
        return None
...
def handle_resume_update(
    ...
    try:
        ...
        logger.error(
            f"[UPDATE_RESUME] Failed for resume_id: {resume_id} - {str(e)}",
            exc_info=True,
        )
        raise
